...
/*fun intializeViewModel(clazz: KClass<T>){

    var apiRepository= APIRepository()
    var viewModelFactory= ViewModelProvider.
...
fun showDialogConfirmation(
    ...
    buttonAction: (String, DialogInterface) -> Unit
) {
        ...
        builder.setPositiveButton("OK") { dialogInterface, which ->
            ...
            buttonAction.invoke("positive", dialogInterface)
        }
...
fun showSnack(activity: Activity, text: String)
{
 ...
 suspend fun get(bitmap: Bitmap, fileNameToSave: String, context: Context) :File?{
  ...
  suspend fun  bitmapToFile(bitmap: Bitmap, fileNameToSave: String, context: Context): File?=get(
      bitmap,
      fileNameToSave,
      context
  )
...
fun requestMultiplePermission(
    resultLauncher: ActivityResultLauncher<Array<out String>>,
...
{
    showProgressDialog(context, "Fetching relations")
...
suspend fun  loadJSONFromAsset(context: Context): String {
    var json: String
    json = try {
    ...
    fun getStateCityList(context: Context, json: String, isState: Boolean,key:String):ArrayList<String>{
        var states=ArrayList<String>()
        val obj = JSONObject(json)
        if(isState) {

            var iterator: Iterator<String> = obj.keys()
            ...
            states = gson.fromJson<ArrayList<String>>((obj.getString(key)), object :
